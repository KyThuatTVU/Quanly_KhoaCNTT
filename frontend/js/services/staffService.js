...
export const StaffService = {
  ...
  async getStaffList() {
    try {
      const result = await fetchWithCache(`${BASE_URL}/staff`);
  ...
  async getStaffGroups() {
    try {
      const result = await fetchWithCache(`${BASE_URL}/staffGroups`);
      if (result && result.success && Array.isArray(result.data)) {
        return result.data;
      }
    } catch (e) {
  ...
  async getStaffProfile(staffId) {
    try {
      const result = await fetchWithCache(`${BASE_URL}/staffProfiles`);
      if (result && result.success && Array.isArray(result.data)) {
  ...
  async getStaffResearchProjects(staffId) {
    try {
      const result = await fetchWithCache(`${BASE_URL}/staffResearch`);
      if (result && result.success && Array.isArray(result.data)) {
  ...
  async getStaffProjects(staffId) {
    try {
      const result = await fetchWithCache(`${BASE_URL}/staffProjects`);
      if (result && result.success && Array.isArray(result.data)) {
        return result.data.filter(p => parseInt(p.nhan_vien_id, 10) === parseInt(staffId, 10));
      }
    } catch (e) {
  ...
  async getStaffPublications(staffId) {
    try {
      const result = await fetchWithCache(`${BASE_URL}/staffPapers`);
      if (result && result.success && Array.isArray(result.data)) {
        return result.data.filter(p => parseInt(p.nhan_vien_id, 10) === parseInt(staffId, 10));
      }
    } catch (e) {
  ...
  async getStaffBooks(staffId) {
    try {
      const result = await fetchWithCache(`${BASE_URL}/staffBooks`);
      if (result && result.success && Array.isArray(result.data)) {
  ...
  async getStaffSupervisions(staffId) {
    try {
      const result = await fetchWithCache(`${BASE_URL}/staffSupervisions`);
      if (result && result.success && Array.isArray(result.data)) {
